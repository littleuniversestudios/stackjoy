...
class TemplateService {
    ...
    create({ name, collectionId, files, newCollectionName }) {
        if (!!newCollectionName && !collectionId) {
            const result = globals_1.APP_ENVIRONMENT.getBlueprints().createCollection(newCollectionName);
            collectionId = result.data.collection.id;
        }
        return globals_1.APP_ENVIRONMENT.getBlueprints().createTemplate(name, collectionId, files);
    }
    rename({ newName, templateId }) {
        return globals_1.APP_ENVIRONMENT.getBlueprints().renameTemplate(newName, templateId);
    }
    delete(id) {
        return globals_1.APP_ENVIRONMENT.getBlueprints().deleteTemplate(id);
    }
    duplicate(id) {
