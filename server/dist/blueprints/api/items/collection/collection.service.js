...
class CollectionService {
    ...
    // }
    create({ name }) {
        const result = globals_1.APP_SERVICE.CURRENT_ENVIRONMENT.getBlueprints().createCollection(name);
        return { error: result.error, data: { success: result.data.success } };
    }
    rename(id, { newName }) {
        return globals_1.APP_SERVICE.CURRENT_ENVIRONMENT.getBlueprints().renameCollection(id, newName);
    }
    deleteCollection(id) {
        return globals_1.APP_SERVICE.CURRENT_ENVIRONMENT.getBlueprints().deleteCollection(id);
    }
}
