...
class AppModel {
    ...
    ensureSystemWorkspace() {
        if (!fs_extra_1.existsSync(path_1.join(this.systemWorkspacePath, 'metadata.json'))) {
            const systemEnvironmentMetadata = this.createSystemEnvironment();
            environment_model_1.EnvironmentModel.createDefaultCollection(new environment_model_1.EnvironmentModel(systemEnvironmentMetadata));
        }
        this.systemWorkspace = environment_model_1.EnvironmentModel.getEnvironmentMetadata(this.systemWorkspacePath);
    ...
    createSystemEnvironment() {
        const environmentRelativePath = this.systemWorkspaceFolder;
        return environment_model_1.EnvironmentModel.createEnvironment(environmentRelativePath, {
            id: util_1.UUIDShort(),
            codebasePath: null,
