...
class AppUserModel {
    ...
    getListOfEnvironments(environmentPath) {
        ...
        environmentDirs.forEach(dir => environments.push(app_environment_model_1.EnvironmentModel.getEnvironmentMetadata(path_1.join(environmentPath, dir))));
        return environments;
    }
    ...
    createEnvironment(codebasePath, name, type, isLocal, id = util_1.UUIDShort()) {
        return app_environment_model_1.EnvironmentModel.createEnvironment({
            codebasePath, name, type, isLocal, id,
            environmentPath: path_1.join(type === app_interface_1.App.Environment.Type.Workspace ? this.workspacesPath : this.stacksPath, id)
        });
    }
}
